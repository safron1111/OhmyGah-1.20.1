...
public class NuclearBomb extends TntBlock {
    ...
    public NuclearBomb(Settings settings) {
        super(settings);
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (!world.isClient) {
            NuclearBombEntity nuclearBombEntity = new NuclearBombEntity(world, (double)pos.getX() + 0.5, (double)pos.getY(), (double)pos.getZ() + 0.5, explosion.getCausingEntity());
            int i = nuclearBombEntity.getFuse();
            nuclearBombEntity.setFuse((short)(world.random.nextInt(i / 4) + i / 8));
    ...
    private static void primeExplosive(@NotNull World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            NuclearBombEntity nuclearBombEntity = new NuclearBombEntity(world, (double)pos.getX() + 0.5, (double)pos.getY(), (double)pos.getZ() + 0.5, igniter);
            world.spawnEntity(nuclearBombEntity);
            world.playSound(null, nuclearBombEntity.getX(), nuclearBombEntity.getY(), nuclearBombEntity.getZ(), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, 1.0F);
